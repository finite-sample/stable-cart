...
def friedman1_dataset(n_samples: int = 3000, noise: float = 1.0, random_state: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def friedman2_dataset(n_samples: int = 3000, noise: float = 1.0, random_state: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def friedman3_dataset(n_samples: int = 3000, noise: float = 1.0, random_state: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def quadrant_interaction_dataset(n_samples: int = 3000, noise: float = 0.5, random_state: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def high_dimensional_sparse_dataset(n_samples: int = 3000, n_features: int = 50, 
                                  n_informative: int = 5, noise: float = 1.0, 
...
def heteroscedastic_dataset(n_samples: int = 3000, noise_scale: float = 1.0, 
                          random_state: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    # Base signal
    y = (2.0 * X[:, 0] - 1.5 * X[:, 1] + 0.8 * X[:, 2] * X[:, 3] + 
         0.5 * np.tanh(X[:, 4]) - 0.3 * np.abs(X[:, 5]))
...
def xor_nonlinear_dataset(n_samples: int = 3000, n_features: int = 8, 
                         noise: float = 0.7, random_state: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def get_dataset_info() -> pd.DataFrame:
    ...
    for name, func in REGRESSION_DATASETS.items():
        ...
        info.append({
            ...
            'description': func.__doc__.split('\n')[1].strip() if func.__doc__ and len(func.__doc__.split('\n')) > 1 else func.__doc__.strip() if func.__doc__ else ''
        })
...
def load_dataset(name: str, test_size: float = 0.3, random_state: int = 42) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, str]:
    """
...
def get_dataset_recommendations() -> Dict[str, list]:
    ...
    return {
        ...
        'stability_showcase': [
            'quadrant_interaction', 'heteroscedastic', 'xor_nonlinear',
            'breast_cancer', 'digits_binary'
        ],
        'real_world_only': [
            'california_housing', 'diabetes', 'breast_cancer', 
            'wine', 'iris', 'digits_multiclass'
        ]
    }

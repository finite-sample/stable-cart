...
def get_regression_models(random_state: int = 42) -> Dict[str, Callable]:
    """Get factory functions for regression models."""
    return {
        ...
        'LessGreedyHybrid': lambda: LessGreedyHybridRegressor(
            ...
            enable_oblique_root=True, gain_margin=0.03,
            beam_topk=12, leaf_shrinkage_lambda=10.0,
            random_state=random_state
        ),
        'BootstrapVariancePenalized': lambda: BootstrapVariancePenalizedRegressor(
            max_depth=6, min_samples_split=40, min_samples_leaf=20,
            split_frac=0.6, val_frac=0.2, est_frac=0.2,
            variance_penalty=1.0, n_bootstrap=10,
            beam_topk=12, leaf_shrinkage_lambda=10.0,
            random_state=random_state
        )
    }
...
def get_classification_models(random_state: int = 42) -> Dict[str, Callable]:
    """Get factory functions for classification models."""
    return {
        ...
        'RobustPrefixHonest': lambda: RobustPrefixHonestClassifier(
            top_levels=2, max_depth=6, min_samples_leaf=20,
            val_frac=0.2, est_frac=0.4, m_smooth=1.0,
            consensus_B=12, random_state=random_state
        )
    }
    ...
    try:
        ...
        if hasattr(model, 'tree_'):
            ...
            if hasattr(model, 'estimators_'):
                avg_leaves = np.mean([np.sum(tree.tree_.children_left == -1) for tree in model.estimators_])
...
def evaluate_dataset(
    ...
    # Get appropriate model factories
    if task == 'regression':
        model_factories = get_regression_models(random_state)
    else:
        model_factories = get_classification_models(random_state)
...
if __name__ == "__main__":
    ...
    results = evaluate_single_model(
        'TestCART', factory, X_train, y_train, X_test, y_test, 
        'classification', n_bootstrap=5
    )

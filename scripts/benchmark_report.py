...
def create_stability_summary_table(stability_df: pd.DataFrame) -> str:
    ...
    model_summary = stability_df.groupby('model').agg({
...
def create_dataset_insights(results_df: pd.DataFrame, stability_df: pd.DataFrame) -> str:
    ...
    for dataset, improvement in worst_datasets.items():
        if improvement < 0:
            insights.append(f"- **{dataset}**: Stable methods show {abs(improvement):.1f}% worse variance")
        else:
...
def generate_markdown_report(
    ...
    if not stability_df.empty:
        ...
        if len(stable_models) > 0:
            avg_improvement = stability_df[
                stability_df['model'].isin(stable_models)
            ]['variance_reduction_pct'].mean()
            ...
            best_stable = stability_df[
                stability_df['model'] != 'CART'
            ].groupby('model')['variance_reduction_pct'].mean().idxmax()
            best_improvement = stability_df[
                stability_df['model'] == best_stable
            ]['variance_reduction_pct'].mean()
    ...
    report_lines.extend([
        "## Dataset Insights",
        "",
        create_dataset_insights(results_df, stability_df)
    ])
    ...
    if not stability_df.empty:
        ...
        positive_models = stable_models[stable_models['variance_reduction_pct'] > 0]['model'].unique()
        ...
        if len(positive_models) > 0:
            ...
            for model in positive_models:
                avg_improvement = stable_models[
                    stable_models['model'] == model
                ]['variance_reduction_pct'].mean()
                report_lines.append(f"- **{model}**: {avg_improvement:.1f}% average variance reduction")
        else:
            report_lines.extend([
...
if __name__ == "__main__":
    ...
    dummy_results = pd.DataFrame([
        {'dataset': 'test', 'model': 'CART', 'task': 'regression', 
         'pred_variance_mean': 0.5, 'mse': 0.1, 'r2': 0.8},
        {'dataset': 'test', 'model': 'LessGreedyHybrid', 'task': 'regression',
         'pred_variance_mean': 0.3, 'mse': 0.12, 'r2': 0.78}
    ])
    
    dummy_stability = pd.DataFrame([
        {'dataset': 'test', 'model': 'CART', 'variance_reduction_pct': 0.0, 'relative_variance': 1.0},
        {'dataset': 'test', 'model': 'LessGreedyHybrid', 'variance_reduction_pct': 40.0, 'relative_variance': 0.6}
    ])
    
    config = {'random_state': 42, 'n_bootstrap': 20, 'datasets': ['test']}
    
    report_path = generate_markdown_report(
        dummy_results, pd.DataFrame(), dummy_stability, 
        "./test_output", config
    )

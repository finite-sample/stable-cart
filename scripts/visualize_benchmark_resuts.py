...
def plot_stability_comparison(df_stab, output_dir):
    ...
    ax1.set_title('Mean Prediction Std Dev (Lower = More Stable)', 
    ...
    ax2.set_title('P90 Prediction Std Dev (Lower = More Stable)', 
                  fontsize=14, fontweight='bold')
    ax2.set_xlabel('Dataset', fontsize=12)
...
def plot_relative_improvements(df_stab, output_dir):
    ...
    for dataset in datasets:
        cart_var = df_stab[
            (df_stab.dataset == dataset) & (df_stab.model == 'CART')
    ...
    ax.set_title('Prediction Stability Improvement Over CART Baseline', 
                 fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
def plot_tree_size_comparison(df_acc, output_dir):
    ...
    ax.set_title('Tree Size Comparison (Number of Leaves)', 
                 fontsize=14, fontweight='bold')
    ax.set_xlabel('Dataset', fontsize=12)
...
def generate_summary_table(df_acc, df_stab, output_dir):
    ...
    acc_summary = df_acc.groupby('model').agg({
        'mse': 'mean',
        'r2': 'mean',
        'leaves': 'mean',
        'fit_time_sec': 'mean'
    ...
    stab_summary = df_stab.groupby('model').agg({
        'mean_pred_std': 'mean',
        'p90_pred_std': 'mean'
    }).round(4)
    ...
    # Reorder columns
    summary = summary[[
        'mse', 'r2', 'mean_pred_std', 'stability_vs_cart_pct', 
        'leaves', 'fit_time_sec'
    ]]

...
A single-tree regressor that trades a bit of accuracy for *stability* via:
 - Honest data partitioning at fit-time: SPLIT (candidate generation), VAL (honest scoring), EST (leaf prediction)
 - Optional oblique (linear) *root* split with gating (correlation + margin vs axis)
 - Optional honest k-step lookahead (axis-only) with beam search, scheduled near the top when gains are ambiguous
...
class GreedyCARTExact(BaseEstimator, RegressorMixin):
    ...
    def _children_sse_vec(self, xs, ys, min_leaf):
        n=ys.size
        ps1=np.cumsum(ys, dtype=np.float64); ps2=np.cumsum(ys*ys, dtype=np.float64)
        tot1=ps1[-1]; tot2=ps2[-1]
        idx=np.arange(n-1)
        valid=(xs[:-1]!=xs[1:])
        nL=idx+1; nR=n-nL
        valid &= (nL>=min_leaf) & (nR>=min_leaf)
        sumL=ps1[idx]; sumL2=ps2[idx]
        sumR=tot1-sumL; sumR2=tot2-sumL2
    ...
    def _build(self, X, y, depth):
        ...
        if sp is None: return {'type':'leaf','value':float(y.mean()),'n':int(n)}
        left = X[:,sp.feature] <= sp.threshold
        if left.sum()<self.min_samples_leaf or (n-int(left.sum()))<self.min_samples_leaf:
            return {'type':'leaf','value':float(y.mean()),'n':int(n)}
        return {'type':'split','f':int(sp.feature),'t':float(sp.threshold),'gain':float(sp.gain),'n':int(n),
                'left': self._build(X[left],y[left],depth+1),
                'right': self._build(X[~left],y[~left],depth+1)}

    def fit(self, X, y):
        t0=time.time(); self.splits_scanned_=0
        self.tree_=self._build(np.asarray(X), np.asarray(y), depth=0); self.fit_time_sec_=time.time()-t0
        return self
...
class LessGreedyHybridRegressor(BaseEstimator, RegressorMixin):
    ...
    def _topk_axis_candidates(self, Xs, ys, topk):
        ...
        p = Xs.shape[1]
        for j in range(p):
            order=np.argsort(Xs[:,j], kind='mergesort'); xs = Xs[order, j]; ys_ord=ys[order]
    ...
    def _val_sse_leaf(self, yv): return _sse(yv)
    ...
    def _best_kstep_val_sse(self, Xs, ys, Xv, yv, depth_remaining, topk):
        ...
        if not cand: return self._val_sse_leaf(yv)
        best = np.inf
        for _, f, t in cand:
            mask_s = Xs[:, f] <= t
            if mask_s.sum() < self.min_samples_leaf or (ys.size - int(mask_s.sum())) < self.min_samples_leaf:
                continue
            mask_v = Xv[:, f] <= t
            sseL = self._best_kstep_val_sse(Xs[mask_s], ys[mask_s], Xv[mask_v], yv[mask_v], depth_remaining-1, topk)
            sseR = self._best_kstep_val_sse(Xs[~mask_s], ys[~mask_s], Xv[~mask_v], yv[~mask_v], depth_remaining-1, topk)
            tot = sseL + sseR
            if tot < best: best = tot
        if not np.isfinite(best): return self._val_sse_leaf(yv)
        return float(best)
    ...
    def _build(self, Xs, ys, Xv, yv, Xe, ye, depth, parent_mean_est):
        ...
        if depth>=self.max_depth or n_split < self.min_samples_split:
            ...
            return {'type':'leaf','value':mu,'n_split':int(n_split),'n_val':int(n_val),'n_est':int(ye.size)}
        ...
        if cand_axis and n_split >= self.min_n_for_lookahead and depth <= 1:
            if len(cand_axis) >= 2:
                g1 = cand_axis[0][0]; g2 = cand_axis[1][0]
        ...
        if k_here > 0 and cand_axis:
            ...
            for _, f, t in cand_axis:
                ...
                if mask_s.sum() < self.min_samples_leaf or (ys.size - int(mask_s.sum())) < self.min_samples_leaf:
                    continue
                mask_v = Xv[:, f] <= t
                sseL = self._best_kstep_val_sse(Xs[mask_s], ys[mask_s], Xv[mask_v], yv[mask_v], k_here-1, self.beam_topk)
                sseR = self._best_kstep_val_sse(Xs[~mask_s], ys[~mask_s], Xv[~mask_v], yv[~mask_v], k_here-1, self.beam_topk)
                tot = sseL + sseR
                if tot < best_la[0]:
                    best_la = (tot, f, t)
        ...
        if self.enable_oblique_root and depth==0 and Xs.shape[1] >= 2:
            ...
            lcv = LassoCV(cv=self.oblique_cv, random_state=self.random_state)
            lcv.fit(Xs_std, ys)
            w = lcv.coef_.astype(float)
            if np.count_nonzero(w) > 0 and max_abs_corr >= self.min_abs_corr:
                s = Xs_std @ w
                order = np.argsort(s, kind='mergesort'); ss=s[order]; ys_ord=ys[order]
                ...
                if valid.any():
                    ...
                    if not np.isfinite(axis_gain_top) or oblique_gain_split >= axis_gain_top*(1.0 + self.gain_margin):
                        # score on VAL
                        ...
                        if sse_obl + 1e-12 < best_val_sse:
                            ...
                            best_info = (float(t), scaler.mean_.astype(float), scaler.scale_.astype(float), w.astype(float))
                            self.oblique_info_ = {'alpha': float(lcv.alpha_), 'nnz': int(np.count_nonzero(w)),
        ...
        if best_kind == 'axis':
            ...
            if mask_s.sum() < self.min_samples_leaf or (ys.size - int(mask_s.sum())) < self.min_samples_leaf:
                ...
                mu = ((ye.size * mu_leaf + lam * parent_mean_est) / (ye.size + lam)) if lam>0 else mu_leaf
                return {'type':'leaf','value':mu,'n_split':int(n_split),'n_val':int(n_val),'n_est':int(ye.size)}
            node = {'type':'split','f':int(f),'t':float(t),'n_split':int(n_split),'n_val':int(n_val),'n_est':int(ye.size)}
            node['left']  = self._build(Xs[mask_s], ys[mask_s], Xv[mask_v], yv[mask_v], Xe[mask_e], ye[mask_e], depth+1,
                                        parent_mean_est=float(ye.mean()) if ye.size>0 else float(ys.mean()))
            node['right'] = self._build(Xs[~mask_s], ys[~mask_s], Xv[~mask_v], yv[~mask_v], Xe[~mask_e], ye[~mask_e], depth+1,
                                        parent_mean_est=float(ye.mean()) if ye.size>0 else float(ys.mean()))
            return node
        ...
        if best_kind == 'oblique_root':
            ...
            node = {'type':'split_oblique','t':float(t),'w':w.astype(float),
                    'scaler_mean':mean.astype(float),'scaler_scale':scale.astype(float),
                    'n_split':int(n_split),'n_val':int(n_val),'n_est':int(ye.size)}
            node['left']  = self._build(Xs[mask_s], ys[mask_s], Xv[mask_v], yv[mask_v], Xe[mask_e], ye[mask_e], depth+1,
                                        parent_mean_est=float(ye.mean()) if ye.size>0 else float(ys.mean()))
            node['right'] = self._build(Xs[~mask_s], ys[~mask_s], Xv[~mask_v], yv[~mask_v], Xe[~mask_e], ye[~mask_e], depth+1,
                                        parent_mean_est=float(ye.mean()) if ye.size>0 else float(ys.mean()))

...
def honest_data_partition(
    ...
    X_val, X_est, y_val, y_est = train_test_split(
        X_temp, y_temp, test_size=est_size_relative, 
...
def _stabilize_classification_leaf(
    ...
    unique_classes = np.unique(np.concatenate([y_est, y_parent])) if len(y_parent) > 0 else np.unique(y_est)
    n_classes = len(unique_classes)
    
    # Leaf counts
    leaf_counts = np.bincount(y_est.astype(int), minlength=n_classes)
    parent_counts = np.bincount(y_parent.astype(int), minlength=n_classes) if len(y_parent) > 0 else leaf_counts
    
    if strategy == 'm_estimate':
        # M-estimate smoothing
        prior = parent_counts / np.sum(parent_counts) if np.sum(parent_counts) > 0 else np.ones(n_classes) / n_classes
        ...
        parent_probs = parent_counts / np.sum(parent_counts) if np.sum(parent_counts) > 0 else np.ones(n_classes) / n_classes
...
def generate_oblique_candidates(
    ...
    try:
        if task == 'regression':
            ...
            model = LogisticRegressionCV(cv=3, random_state=random_state, penalty='l1', solver='liblinear')
            model.fit(X, y)
